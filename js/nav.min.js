const currentYear=new Date().getFullYear();const yearSpan=document.getElementById('year-span');yearSpan.textContent=currentYear;const focusableElements='button, input:not([type="hidden"]), select, textarea, [tabindex]:not([tabindex="-1"])';let firstFocusableElement,lastFocusableElement,focusableCount=0;let navLinks=document.querySelector(".nav-links");let menuOpenBtn=document.querySelector(".navbar .icon-bars");let menuCloseBtn=document.querySelector(".nav-links .icon-close");if(menuOpenBtn){menuOpenBtn.onclick=function(){navLinks.style.left="0"}}
...
applyLazyLoading();window.addEventListener('resize',applyLazyLoading);document.querySelectorAll('a').forEach(link=>{link.addEventListener('click',function(event){if(link.rel.includes('nofollow')){event.preventDefault();const target=link.target;const url=link.href;setTimeout(()=>{if(target==="_blank"){window.open(url,"_blank")}else{window.location.href=url}},500)}})})